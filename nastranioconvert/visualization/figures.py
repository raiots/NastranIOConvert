...
def fig_deformed_overlay_3d(grids: pd.DataFrame, scaled_df: pd.DataFrame, mode: str) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter3d(
            ...
            z=merged["z"],
            mode="markers",
            marker=dict(size=3, color="rgba(100,100,100,0.55)"),
            name="original",
    ...
    fig.add_trace(
        go.Scatter3d(
            x=merged["x"] + merged["ux"],
            y=merged["y"] + merged["uy"],
            z=merged["z"] + merged["uz"],
            mode="markers",
            marker=dict(size=3, color="#d62728", opacity=0.85),
            name="scaled",
            customdata=merged["node_id"],
            hovertemplate="node=%{customdata}<br>x=%{x:.3f}<br>y=%{y:.3f}<br>z=%{z:.3f}<extra></extra>",
        )
